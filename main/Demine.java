...
public class Demine extends JFrame {
    @Serial
                        ...
                        if (mouse < 0) {
                            ...
                            if (play != -1) {
                                switch (status) {
                                    ...
                                    case DOUBT -> setStatus(Status.BUTTON);
                                    default -> {
                                    }
                                }
                            } else {
        ...
        private void clickLabel(Point p) {
            if (label[p.x][p.y].status == Status.BUTTON || label[p.x][p.y].status == Status.DOUBT) {
                if (mark[p.x][p.y] != Status.FAILED.getValue()) {
                    // 未点击到地雷
                    java.util.Queue<Point> que = new LinkedList<>();
                    ...
                    if (open == row * col - mines) {
                        ...
                        if (Integer.parseInt(timecal.getText()) < rank.item[level].score) {
                            String Tip = "<html><font face=\"黑体\">已破";
                            switch (level) {
                                case 0 -> Tip += "初级";
                                case 1 -> Tip += "中级";
                                case 2 -> Tip += "高级";
                            ...
                            buttontemp.addActionListener(e -> success.dispose());
    ...
    class Option extends JDialog {
        ...
        public Option(JFrame frame) {
            ...
            radio[0].addActionListener(e -> select = 0);
            radio[1].addActionListener(e -> select = 1);
            radio[2].addActionListener(e -> select = 2);
    ...
    public Demine() {
        ...
        newGame.addActionListener(e -> panel.reSize(Difficult.Diff[level][0], Difficult.Diff[level][1], Difficult.Diff[level][2]));
        JFrame temp = this;
        JMenuItem stat = new JMenuItem("统计");
        stat.addActionListener(e -> new Stat(temp));
        JMenuItem option = new JMenuItem("选项");
        option.addActionListener(e -> new Option(temp));
